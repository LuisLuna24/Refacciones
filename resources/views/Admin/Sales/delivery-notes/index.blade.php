<x-admin-layout title="Notas de remisión | Inventarios" :breadcrumbs="[
    [
        'name' => 'Dashboard',
    ...
    <div class="flex items-center justify-between mb-8 pb-5 border-b border-gray-200 dark:border-gray-800">
        <div class="min-w-0 flex-1">
            <h1
                ...
                {{ __('Notas de remisión') }}
            </h1>
        </div>

        <div class="my-2 flex md:ml-4 md:mt-0">
            @can('create-sales')
                <x-w-button href="{{ route('admin.delivery_notes.create') }}" blue icon="plus" label="Nuevo"
                    class="shadow-sm hover:shadow-md transition-all duration-200" />
            @endcan
        </div>
    </div>

    @livewire(' admin.datatables.sales.delivery-note-table')
